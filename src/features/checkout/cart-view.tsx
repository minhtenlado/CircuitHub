  ...
  if (items.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        <div className="h-24 w-24 rounded-full bg-red-50 dark:bg-red-950/30 border-2 border-red-200 dark:border-red-800 flex items-center justify-center mx-auto mb-6">
          <ShoppingCart className="h-11 w-11 text-red-400 dark:text-red-600" />
        </div>
                ...
                <AnimatePresence initial={false}>
                  {g.items.map((item) => (
                    <motion.div
                      ...
                      layout
                      exit={{ opacity: 0, x: -20 }}
                      className="flex gap-4 p-2 rounded-lg hover:bg-red-50/20 dark:hover:bg-red-900/10 transition-colors"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-3">
                          <div className="flex items-center border border-border/60 rounded-md">
                            <button onClick={() => updateQty(item.productId, item.quantity - 1)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/30 text-muted-foreground hover:text-red-700 dark:hover:text-red-400 rounded-l-md transition-colors disabled:opacity-30" disabled={item.quantity <= 1}>
                              <Minus className="h-3.5 w-3.5" />
                            </button>
                            <span className="px-3 text-sm font-semibold tabular-nums">{item.quantity}</span>
                            <button onClick={() => updateQty(item.productId, item.quantity + 1)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/30 text-muted-foreground hover:text-red-700 dark:hover:text-red-400 rounded-r-md transition-colors">
                              <Plus className="h-3.5 w-3.5" />
                            </button>
                          </div>
                          <span className="font-bold text-red-700 dark:text-red-400 tabular-nums">{formatVND(item.price * item.quantity)}</span>
          ...
          <Card className="lg:sticky lg:top-32 border-border/60 bg-card dark:bg-slate-900">
            <CardContent className="p-5 space-y-4">
              ...
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-xl font-bold text-red-700 dark:text-red-400 tabular-nums">{formatVND(total)}</span>
              </div>
              ...
              {!appliedVoucher ? (
                <div className="flex gap-2 pt-1">
                  <div className="relative flex-1">
                    ...
                    <Input value={voucherCode} onChange={(e) => setVoucherCode(e.target.value)} placeholder="Voucher" className="pl-8 h-9 text-sm bg-background text-foreground focus-visible:ring-red-400 dark:focus-visible:ring-red-900" />
                  </div>
              ...
              <Button
                ...
                className="w-full bg-red-600 hover:bg-red-700 text-white dark:bg-red-700 dark:hover:bg-red-600 cursor-pointer"
              >
                <Lock className="h-4 w-4 mr-2" />
                {t('cart.checkout')}
              </Button>
              <Button onClick={() => goProducts()} variant="outline" className="w-full border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30">
