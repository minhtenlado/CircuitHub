        ...
        {items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6 py-12">
            <div className="h-20 w-20 rounded-full bg-cyan-50 border border-cyan-200 flex items-center justify-center">
              <ShoppingCart className="h-9 w-9 text-cyan-400" />
            </div>
            ...
            <Button onClick={handleBrowse} className="bg-cyan-600 hover:bg-cyan-700 text-white">
          ...
          <>
            ...
            {/* Voucher + Footer */}
            <div className="border-t border-border/60 bg-slate-50/40 px-4 py-3 space-y-3">
              {/* Voucher */}
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full h-9 pl-8 pr-3 text-sm bg-white rounded-md border border-border/60 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-100 outline-none"
                  />
                </div>
                <Button
                  size="sm"
              ...
              {appliedVoucher && (
                <div className="flex items-center justify-between text-xs bg-emerald-50 border border-emerald-200 rounded-md px-3 py-1.5">
                  ...
                  <span className="font-semibold text-emerald-700">−{formatVND(appliedVoucher.discount)}</span>
                  <button
                    onClick={() => {
                      setAppliedVoucher(null);
                      setVoucherCode('');
              ...
              <div className="space-y-1 text-sm">
                ...
                <div className="flex justify-between pt-1 border-t border-border/40">
                  ...
                  <span className="text-lg font-bold text-cyan-700 tabular-nums">{formatVND(finalTotal)}</span>
                </div>
              </div>

              <div className="flex gap-2">
                <Button onClick={handleCheckout} className="flex-1 bg-cyan-600 hover:bg-cyan-700 text-white">
                  <Lock className="h-3.5 w-3.5 mr-1.5" />
                  Checkout
                </Button>
                <Button onClick={handleBrowse} variant="outline" className="border-cyan-200 text-cyan-700 hover:bg-cyan-50">
                  Continue
                </Button>
              </div>
            </div>
